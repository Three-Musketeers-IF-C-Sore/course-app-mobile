import 'package:flutter/material.dart';
 
const Color $primary500 = Color.fromRGBO(91, 80, 216, 1.0);
const Color $primary300 = Color.fromRGBO(130, 122, 225, 1.0);
const Color $secondary500 = Color.fromRGBO(83, 177, 118, 1.0);
const Color $tertiary300 = Color.fromRGBO(161, 161, 161, 1.0);
const Color $tertiary500 = Color.fromRGBO(109, 109, 109, 1.0);
const Color $tertiary800 = Color.fromRGBO(59, 59, 59, 1.0);
const Color $black = Color.fromRGBO(0,0,0,1.0);
const Color $white = Color.fromRGBO(255,255,255,1.0);
const Color $readMode = Color.fromARGB(255, 255, 232, 211);
const Color $darkMode = Color.fromARGB(255, 27, 23, 64);

